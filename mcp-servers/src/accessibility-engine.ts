...
import type { WcagLevel, CriterionResult } from "./wcag-criteria.js";
...
function scoreFromFindings(passCount: number, failCount: number): number {
...
function mapAxeTagsToLevel(tags: string[]): WcagLevel | null {
  for (const tag of tags) {
    if (tag === "wcag2aaa" || tag === "wcag22aaa" || tag === "wcag21aaa") return "AAA";
    if (tag === "wcag2aa" || tag === "wcag22aa" || tag === "wcag21aa") return "AA";
    if (tag === "wcag2a" || tag === "wcag22a" || tag === "wcag21a") return "A";
  }
  return null;
}

async function analyzeWithAxe(html: string): Promise<Map<string, CriterionResult> | null> {
  if (!axeCore) return null;
  try {
    const results: AxeResults = await axeCore.default.run({ raw: html } as any);

    const map = new Map<string, CriterionResult>();

    for (const criterion of WCAG_CRITERIA) {
      if (!criterion.axeRule) continue;
      ...
      for (const v of violations) {
        ...
        if (v.helpUrl) recommendations.push(`See ${v.helpUrl}`);
      }
...
// F1: Alt Text (1.1.1)
function scoreAltText(html: string): CriterionResult[] {
...
function scoreMediaAndAudio(_html: string): CriterionResult[] {
  const ids = ["1.2.1","1.2.2","1.2.3","1.2.4","1.2.5","1.2.6","1.2.7","1.2.8","1.2.9","1.4.2","1.4.7"];
  return ids.map(id => {
    const c = WCAG_CRITERIA.find(x => x.id === id)!;
    return {
      id: c.id, title: c.title, level: c.level, principle: c.principle, automatable: "no" as const,
...
function scoreSemanticStructure(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreAdaptable(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreColorContrast(html: string, level: WcagLevel): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreTextPresentation(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreReflow(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreKeyboardAccess(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreTimingAndMotion(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreNavigation(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreInputModalities(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreFormsAndInput(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function scoreDocumentStructure(html: string): CriterionResult[] {
  const results: CriterionResult[] = [];
...
function mergeEngineResults(
  axeResults: Map<string, CriterionResult> | null,
  regexResults: CriterionResult[],
  level: WcagLevel
): CriterionResult[] {
  const levelHierarchy: Record<WcagLevel, number> = { A: 1, AA: 2, AAA: 3 };
