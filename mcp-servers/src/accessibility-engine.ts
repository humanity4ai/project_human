...
export interface AccessibilityScoreResult {
  ...
  heuristic: boolean; // true if input was description (not actual HTML)
...
function scoreColorContrast(html: string, level: AccessibilityLevel): CriterionScore {
...
function scoreKeyboardNavigation(html: string): CriterionScore {
  const findings: string[] = [];
  const recommendations: string[] = [];
  let passCount = 0;
  let failCount = 0;
...
function scoreSemanticHtml(html: string): CriterionScore {
  const findings: string[] = [];
  const recommendations: string[] = [];
  let passCount = 0;
  let failCount = 0;
...
function scoreAriaAttributes(html: string): CriterionScore {
  const findings: string[] = [];
  const recommendations: string[] = [];
  let passCount = 0;
  let failCount = 0;
...
function scoreFormLabels(html: string): CriterionScore {
  const findings: string[] = [];
  const recommendations: string[] = [];
  let passCount = 0;
  let failCount = 0;
...
function scoreHeadingStructure(html: string): CriterionScore {
  const findings: string[] = [];
  const recommendations: string[] = [];
  let passCount = 0;
  let failCount = 0;
  ...
  if (h2Count > 0) passCount += 2;
  if (h3Count > 0) passCount += 1;
...
function scoreFocusOrder(html: string): CriterionScore {
  const findings: string[] = [];
  const recommendations: string[] = [];
  let passCount = 0;
  let failCount = 0;
...
export function assessAccessibility(
  ...
  const criteria: CriterionScore[] = isHtml
    ? [
  ...
  return {
    ...
    criteria,
    summary: isHtml
      ? `Accessibility audit complete. Overall ${level} score: ${aggregateScore}/100. ${
          aggregateScore >= 80 ? "Good baseline — address findings for improvements." :
