...
describe("loadSchema via validateInput", () => {
  ...
  it("V-2: non-existent schema path returns valid:false with error message", () => {
    const result = validateInput("schemas/does-not-exist.json", {});
    expect(result.valid).toBe(false);
    expect((result as { errors: string[] }).errors[0]).toContain("not found");
  });

  it("V-3: malformed schema path returns valid:false with error message", () => {
