...
function boundaryForAction(action) {
    ...
    if (!contract)
        return "Unknown action — no boundary metadata available";
    return contract.safetyBoundary;
}
function inputSchemaPathForAction(action) {
    const contract = actionContracts.find((item) => item.action === action);
...
function str(input, key, fallback = "") {
    const v = input[key];
...
// ─────────────────────────────────────────────
function handleDepressionSensitiveRewrite(input, boundaryNotice) {
    ...
    for (const p of shamePatterns) {
        if (lowerText.includes(p))
            safetyFlags.push(`Shame/blame language detected: "${p}"`);
    }
    for (const p of urgencyPatterns) {
        if (lowerText.includes(p))
            safetyFlags.push(`Urgency pressure detected: "${p}"`);
    }
    for (const p of cognitiveLoadPatterns) {
        if (lowerText.includes(p))
            safetyFlags.push(`High cognitive load pattern detected: "${p}"`);
    }
    for (const p of STIGMA_PATTERNS) {
        if (lowerText.includes(p))
            safetyFlags.push(`Stigmatizing language detected: "${p}"`);
    }
    for (const p of MEDICAL_CLAIM_PATTERNS) {
        if (lowerText.includes(p))
            safetyFlags.push(`Potential medical claim detected: "${p}"`);
    }
    for (const p of MINIMIZING_PATTERNS) {
        if (lowerText.includes(p))
            safetyFlags.push(`Minimizing/invalidating language detected: "${p}"`);
    }
    for (const p of JUDGMENTAL_PATTERNS) {
        if (lowerText.includes(p))
            safetyFlags.push(`Judgmental language detected: "${p}"`);
    }
    for (const p of STIGMATIZING_VERB_PATTERNS) {
        if (lowerText.includes(p))
            safetyFlags.push(`Stigmatizing framing detected: "${p}"`);
    }
    for (const p of CRISIS_LANGUAGE_PATTERNS) {
        if (lowerText.includes(p))
            safetyFlags.push(`Crisis language framing detected: "${p}"`);
    }
    let result;
    if (mode === "audit") {
...
// ─────────────────────────────────────────────
function handleCognitiveAccessibility(input, boundaryNotice) {
...
// ─────────────────────────────────────────────
function handleCulturalContext(input, boundaryNotice) {
...
// ─────────────────────────────────────────────
function handleDeescalation(input, boundaryNotice) {
...
// ─────────────────────────────────────────────
function handleEmpatheticReframe(input, boundaryNotice) {
...
function handleNeurodiversityDesign(input, boundaryNotice) {
    ...
    const checkFocus = (term) => allFocus.length === 0 || allFocus.some(f => f.includes(term));
...
// ─────────────────────────────────────────────
function handleAgeInclusiveDesign(input, boundaryNotice) {
    const flowDescription = str(input, "flow_description");
    const ageGroups = Array.isArray(input["age_groups"]) ? input["age_groups"] : [];
    const recommendations = [];
    const accessNotes = [];
    const lower = flowDescription.toLowerCase();
    const groups = ageGroups.map(g => g.toLowerCase());
    const includesGroup = (term) => groups.length === 0 || groups.some(g => g.includes(term));
...
export function invokeAction(action, input) {
    ...
    switch (action) {
        ...
        case "supportive_reply": {
            ...
            const escalation = riskLevel === "high"
                ? [
