...
// ─── helpers ──────────────────────────────────────────────────────────────────
function makeSchema(required, properties) {
...
describe("validateField — type checks via validateInput", () => {
    it("V-4: string type — valid value passes", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: "hello", risk_level: "low" });
        expect(result.valid).toBe(true);
    });
    it("V-5: string type — number value fails", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: 42, risk_level: "low" });
        expect(result.valid).toBe(false);
        if (!result.valid) {
    ...
    it("V-6: enum valid value passes", () => {
        const result = validateInput("schemas/conflict-de-escalation.input.json", { situation: "dispute", intensity: "medium" });
        expect(result.valid).toBe(true);
    });
    it("V-7: enum invalid value fails with list of valid options", () => {
        const result = validateInput("schemas/conflict-de-escalation.input.json", { situation: "dispute", intensity: "critical" });
        expect(result.valid).toBe(false);
        if (!result.valid) {
    ...
    it("V-8: array type — valid array passes", () => {
        const result = validateInput("schemas/neurodiversity-aware-design.input.json", { ui_description: "animated dashboard", focus: ["adhd"] });
        expect(result.valid).toBe(true);
    });
...
describe("validateInput — required field checks", () => {
    it("V-10: all required fields present returns valid:true", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: "I feel overwhelmed", risk_level: "medium" });
        expect(result.valid).toBe(true);
    });
    it("V-11: missing required field returns valid:false with error", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: "hello" });
        expect(result.valid).toBe(false);
        if (!result.valid) {
    ...
    it("V-12: required field present but empty string is treated as missing", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: "", risk_level: "low" });
        expect(result.valid).toBe(false);
        if (!result.valid) {
    ...
    it("V-13: required field is null returns valid:false", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: null, risk_level: "low" });
        expect(result.valid).toBe(false);
    });
    it("V-14: required field is undefined returns valid:false", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", { message: undefined, risk_level: "low" });
        expect(result.valid).toBe(false);
    });
    ...
    it("V-17: multiple missing required fields — all reported", () => {
        const result = validateInput("schemas/supportive-conversation.input.json", {});
        expect(result.valid).toBe(false);
        if (!result.valid) {
...
describe("validateInput — cultural-sensitivity schema", () => {
    it("V-23: all required fields present passes", () => {
        const result = validateInput("schemas/cultural-sensitivity.input.json", { message: "Hello", audience: "enterprise" });
        expect(result.valid).toBe(true);
    });
    it("V-24: missing message fails", () => {
        const result = validateInput("schemas/cultural-sensitivity.input.json", { audience: "enterprise" });
        expect(result.valid).toBe(false);
    });
});
